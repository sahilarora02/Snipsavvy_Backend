...
import { CREATE_CATEGORY, FETCH_CATEGORIES_BY_WORKSPACE } from "../services/category.service";
...
export async function fetchCategoriesByWorkspace(req: Request, res: Response) {
    ...
    logger.info(`REQ : Fetch all categories for a particular workspace => ${workspace_id}`);

    try {
        const categories = await FETCH_CATEGORIES_BY_WORKSPACE(workspace_id);
        ...
        logger.info(`RESP : Categories fetched => ${categories}`);
